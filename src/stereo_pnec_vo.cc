...
StereoObservationMap MatchStereo(const cv::Mat &left_image,
  ...
  for (size_t i = 0; i < ids.size(); ++i) {
    ...
    observation.right_pixel =
        Eigen::Vector2d(static_cast<double>(right.x),
...
int main(int argc, const char *argv[]) {
  ...
  pnec_options.use_nec_ = false;
  pnec_options.noise_frame_ = pnec::common::Both;
  pnec_options.ceres_init_mode_ =
      pnec::rel_pose_estimation::CeresInitMode::NECCeres;
  pnec_options.use_ceres_ = true;
  ...
  pnec::rel_pose_estimation::PNEC pnec_solver(pnec_options);
  ...
  for (const auto &image : loader) {
    ...
    std::vector<Eigen::Matrix3d> projected_covariances;
    host_bvs.reserve(matches.size());
    target_bvs.reserve(matches.size());
    host_covariances.reserve(matches.size());
    target_covariances.reserve(matches.size());
    projected_covariances.reserve(matches.size());
    ...
    for (const auto &match : matches) {
      ...
      host_covariances.push_back(host_keypoint.bv_covariance_);
      target_covariances.push_back(target_keypoint.bv_covariance_);
      projected_covariances.push_back(target_keypoint.bv_covariance_);
      stereo_supported_matches.push_back(match);
    }

    if (stereo_supported_matches.size() <
        static_cast<size_t>(pnec_options.min_matches_)) {
      continue;
    }

    std::vector<int> inliers;
    Sophus::SE3d pnec_pose = pnec_solver.Solve(
        host_bvs, target_bvs, projected_covariances, host_covariances,
        target_covariances, prev_rel_pose, inliers, frame_timing);
    if (inliers.size() < static_cast<size_t>(pnec_options.min_inliers_)) {
      continue;
    }

    const Eigen::Vector3d metric_translation = EstimateMetricTranslation(
        pnec_pose.so3(), stereo_supported_matches, inliers, prev_stereo,
        curr_stereo);
    Sophus::SE3d rel_pose(pnec_pose.so3(), metric_translation);
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Processed stereo PNEC frame " << curr_frame->id() << " in "
