...
def save_plot(output_path: Path, title: str, gt_positions: np.ndarray, est_positions: np.ndarray) -> None:
    ...
    ax.plot(est_positions[:, 0], est_positions[:, 2], label="Stereo NEC (Sim3 aligned)", linewidth=1.4)
    ax.set_xlabel("x [m]")
    ax.set_ylabel("z [m]")
    ax.set_title(title)
    ax.axis("equal")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch-evaluate KITTI stereo NEC results.")
    parser.add_argument("--results-root", required=True, help="Root directory containing XX_stereo_nec result folders.")
    ...
    for seq in SEQUENCES:
        result_dir = results_root / f"{seq}_stereo_nec"
        ...
        save_plot(
            plot_dir / f"{seq}_stereo_nec_vs_gt.png",
            f"KITTI {seq} Stereo NEC vs GT",
        ...
        rows.append(
            {
                ...
                "plot": str(plot_dir / f"{seq}_stereo_nec_vs_gt.png"),
            }
        )
        print(
    ...
    summary_path = output_dir / "stereo_nec_kitti_summary.csv"
