...
def evaluate_sequence(est_path: Path, gt_path: Path, timing_path: Path) -> dict[str, float]:
    ...
    scale, R, t = umeyama_alignment(est_positions, gt_positions)

    est_sim3 = []
    for pose in est_first_aligned:
        T = np.eye(4)
        T[:3, :3] = R @ pose[:3, :3]
        T[:3, 3] = scale * (R @ pose[:3, 3]) + t
        est_sim3.append(T)
...
def save_plot(output_path: Path, title: str, gt_positions: np.ndarray, est_positions: np.ndarray) -> None:
    ...
    ax.plot(est_positions[:, 0], est_positions[:, 2], label="Stereo PNEC (Sim3 aligned)", linewidth=1.4)
    ax.set_xlabel("x [m]")
    ax.set_ylabel("z [m]")
    ax.set_title(title)
    ax.axis("equal")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch-evaluate KITTI stereo PNEC results.")
    parser.add_argument("--results-root", required=True, help="Root directory containing XX_stereo_pnec result folders.")
    ...
    for seq in SEQUENCES:
        result_dir = results_root / f"{seq}_stereo_pnec"
        ...
        save_plot(
            plot_dir / f"{seq}_stereo_pnec_vs_gt.png",
            f"KITTI {seq} Stereo PNEC vs GT",
        ...
        rows.append(
            {
                ...
                "plot": str(plot_dir / f"{seq}_stereo_pnec_vs_gt.png"),
            }
        )
        print(
    ...
    summary_path = output_dir / "stereo_pnec_kitti_summary.csv"
